    ...
    if m.text == "المطورين":
        if sudo2(m):
            try:
                ...
                if lang is None:
                    await m.reply_text(f"◍ [𝑺𝑨𝑭𝑬𝑬𝑹](tg://user?id={super_sudoers[0]})\n" +
                                       f"◍ [{n.first_name}](tg://user?id={sudoers[0]})\n\n"
                                       "لا يوجد مطورين مرفوعين\n√",
                                       parse_mode=enums.ParseMode.MARKDOWN)
                else:
                    t = "\n◍ قائمة المطورين \n≪━━━━━━━━━━━━━≫\n" + f"◍ [𝑺𝑨𝑭𝑬𝑬𝑹](tg://user?id={super_sudoers[0]})\n" + f"◍ [{n.first_name}](tg://user?id={sudoers[0]})\n\n√"
                    for row in lang:
...
########################################################################################################################
    ...
    if m.text == "السورس" or m.text == "سورس" or m.text == "يا سورس":
        Safeer = await c.get_users(super_sudoers[1])
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                             Safeer.first_name, user_id=super_sudoers[1]
            )
            ], 
            [ InlineKeyboardButton ( "CH SOURCE", url=f"https://t.me/T_N_L_0" )], 
        [InlineKeyboardButton("اضف البوت الي مجموعتك ✅",
                              url=f"https://t.me/{get_bot_information () [1]}?startgroup=new")],
        ])
        await m.reply_text("""
╭──── • ◈ • ────╮
么 [𝗦𝗢𝗨𝗥𝗖𝗘 𝗦𝗔𝗦𝗔](t.me/T_N_L_0)
么 [𝗔𝗦𝗞 𝗧𝗢 𝗠𝗘](@U5_1U)
么 [𝑺𝑨𝑭𝑬𝑬𝑹](t.me/T_N_L_0)
╰──── • ◈ • ────╯
...
⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼
    ...
    if m.text == "سفير":
        s = await c.get_users("7087517667")
        ss = (await c.get_chat("7087517667")).bio
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(f"{s.first_name}", user_id=7087517667)],
        ])
        try:
            await c.get_chat_photos("7087517667", limit=1).__anext__()
        except:
            await m.reply_text(f"{ss}", reply_markup=keyboard),
        else:
            async for photo in c.get_chat_photos("7087517667", limit=1):
    ...
    if m.text == (get_db_botname() or "القائد"):
        texting = ["اؤمر " + (get_db_botname() or "القائد") + " شتريد؟❤️🥺",
                   "اى يقلب " + (get_db_botname() or "القائد") + "❤️",
                   ...
                   "قلبى بينادى على " + (get_db_botname() or "القائد1") + "😘",
                   "نعسان محدش يصحينى🙄"
                   ]
